...
class TestVerySimpleMultiScenario(unittest.TestCase):
    ...
    def test_01_multi_scenario_of_scatter(self):
        ...
        self.assertListEqual([key for key in list(gather_disc3._data_in.keys())if key not in SoSDiscipline.NUM_DESC_IN], [
...
#                 graph.to_plotly().show()
    ...
    def test_04_multi_scenario_of_scatter_parallel(self):
        if os.name == 'nt':
            ...
            gather_disc3 = self.exec_eng.dm.get_disciplines_with_name(
                'MyCase.Post-processing.Disc3')[0]
            self.assertListEqual(sorted([key for key in list(gather_disc3._data_in.keys())if key not in SoSDiscipline.NUM_DESC_IN]), [
                'scenario_1.o', 'scenario_2.o', 'scenario_list'])
            self.assertListEqual(
                list(gather_disc3._data_out.keys()), ['o_dict'])

    def test_05_same_multi_scenario_name_as_study_name(self):

        same_name = 'multi_scenarios'
        self.study_name = same_name
        self.exec_eng = ExecutionEngine(same_name)
        self.factory = self.exec_eng.factory
        # scatter build map
        ac_map = {'input_name': 'name_list',
        ...
        # shared namespace
        self.exec_eng.ns_manager.add_ns('ns_barrierr', f'{same_name}')
        self.exec_eng.ns_manager.add_ns(
            'ns_scatter_scenario', f'{same_name}.multi_scenarios')
        self.exec_eng.ns_manager.add_ns(
            'ns_disc3', f'{same_name}.multi_scenarios.Disc3')
        self.exec_eng.ns_manager.add_ns(
            'ns_out_disc3', f'{same_name}.multi_scenarios')
        self.exec_eng.ns_manager.add_ns(
            'ns_data_ac', f'{same_name}')
        self.exec_eng.ns_manager.add_ns(
            'ns_post_proc', f'{same_name}.Post-processing')
        ...
        multi_scenarios = self.exec_eng.factory.create_very_simple_multi_scenario_builder(
            same_name, 'scenario_list', scatter_list, autogather=True, gather_node='Post-processing')
        ...
        self.assertEqual(self.exec_eng.dm.get_value(
            f'{same_name}.multi_scenarios.scenario_list'), ['scenario_1', 'scenario_2'])

        y1 = a1 * x1 + b1
        y2 = a2 * x2 + b2

        self.assertEqual(self.exec_eng.dm.get_value(
            f'{same_name}.multi_scenarios.scenario_1.name_1.y'), y1)
        self.assertEqual(self.exec_eng.dm.get_value(
            f'{same_name}.multi_scenarios.scenario_1.name_2.y'), y2)
        self.assertEqual(self.exec_eng.dm.get_value(
            f'{same_name}.multi_scenarios.scenario_2.name_1.y'), y1)
        self.assertEqual(self.exec_eng.dm.get_value(
            f'{same_name}.multi_scenarios.scenario_2.name_2.y'), y2)

        gather_disc1 = self.exec_eng.dm.get_disciplines_with_name(
            f'{same_name}.Post-processing.Disc1')[0]
        self.assertListEqual([key for key in list(gather_disc1._data_in.keys()) if key not in gather_disc1.NUM_DESC_IN], [
            'scenario_list', 'scenario_1.y_dict', 'scenario_2.y_dict'])
        self.assertListEqual(list(gather_disc1._data_out.keys()), ['y_dict'])
        self.assertDictEqual(gather_disc1._data_out['y_dict']['value'], {
        ...
        gather_disc3 = self.exec_eng.dm.get_disciplines_with_name(
            f'{same_name}.Post-processing.Disc3')[0]
        self.assertListEqual([key for key in list(gather_disc3._data_in.keys())if key not in SoSDiscipline.NUM_DESC_IN], [
            'scenario_list', 'scenario_1.o', 'scenario_2.o'])
        self.assertListEqual(list(gather_disc3._data_out.keys()), ['o_dict'])
...
if '__main__' == __name__:
    ...
    cls.setUp()
    cls.test_05_same_multi_scenario_name_as_study_name()
