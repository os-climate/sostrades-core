...
class ValueBlockDiscipline(SoSDiscipline):
    """
    ...
    def __init__(self, sos_name, ee, associated_namespaces=[]):
        ...
        super(ValueBlockDiscipline, self).__init__(
            sos_name, ee, associated_namespaces=associated_namespaces)

    def configure(self):
        '''
        Overloaded SoSDiscipline method
        '''
        if self.check_children_are_configured():
            SoSDiscipline.configure(self)

    def check_children_are_configured(self):
        '''
        Check if all children with outputs data to gather are configured
        Return False at least one builder need to be configured
        '''
        for disc in self.children_list:
    ...
    def is_configured(self):
        '''
        Return False at least one children with outputs data to gather need to be configured or structuring variables have changed, True if not
        '''
        return SoSDiscipline.is_configured(self) and self.check_children_are_configured()
    ...
    def add_disc_list_to_children_list(self, disc_list):
        '''
        Add a list to children_list 
        '''
        for disc in disc_list:
            if disc not in self.children_list:
                self.children_list.append(disc)
    ...
    def run(self):
        ...
        for out_key in self._data_out.keys():
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        for output_key, output_value in output_dict.items():
            if output_key.endswith('_gather'):
                ...
                chart_unit = self._data_out[output_key][self.UNIT]
