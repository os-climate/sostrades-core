...
class SobieskiPropulsion(object):
    ...
    def __set_coeff_temp(self, z, x_3):
        ...
        s_new = array([z[2], z[1], x_3[0]], dtype=self.dtype)
    ...
    def compute_sfc(self, z, adim_throttle):
        ...
        sfc = (
            self.sfc_coeff[0]
            + self.sfc_coeff[1] * z[2]
            + self.sfc_coeff[2] * z[1]
            + self.sfc_coeff[3] * throttle
            + self.sfc_coeff[4] * z[2] ** 2
            + 2 * z[1] * z[2] * self.sfc_coeff[5]
            + 2 * throttle * z[2] * self.sfc_coeff[6]
            + self.sfc_coeff[7] * z[1] ** 2
            + 2 * throttle * z[1] * self.sfc_coeff[8]
            + self.sfc_coeff[9] * throttle ** 2
        )
        return sfc

    def compute_throttle_ua(self, z):
        ...
        throttle_ua = (
            self.thua_coeff[0]
            + self.thua_coeff[1] * z[2]
            + self.thua_coeff[2] * z[1]
            + self.thua_coeff[3] * z[2] * z[2]
            + 2 * self.thua_coeff[4] * z[2] * z[1]
            + self.thua_coeff[5] * z[1] * z[1]
        )
        return throttle_ua

    def compute_throttle_constraint(self, z, adim_throttle):
    ...
    def compute_dthrcons_dh(self, z, adim_throttle):
        ...
        dthrottle_ua_dh = (
            self.thua_coeff[2]
            + 2 * self.thua_coeff[4] * z[2]
            + 2.0 * self.thua_coeff[5] * z[1]
        )
        return -throttle * dthrottle_ua_dh / (throttle_ua * throttle_ua)

    def compute_dthrconst_dmach(self, z, adim_throttle):
        ...
        dthrottle_ua_dmach = (
            self.thua_coeff[1]
            + 2 * self.thua_coeff[3] * z[2]
            + 2.0 * self.thua_coeff[4] * z[1]
        )
    ...
    def compute_temp(self, z, x_3):
        ...
        :rtype: numpy array
        """
        s_initial, s_new, flag, bound = self.__set_coeff_temp(z, x_3)
    ...
    def blackbox_propulsion(self, z, y_23, x_3, true_cstr=False):
        ...
        y_3[0] = self.compute_sfc(z, x_3[0])
        ...
        # engine temperature
        temp = self.compute_temp(z, x_3)
        g_3[1] = temp

        g_3[2] = self.compute_throttle_constraint(z, x_3[0])
    ...
    def compute_dsfc_dthrottle(self, z, adim_throttle):
        ...
        dsfc_dthrottle = (
            self.sfc_coeff[3]
            + 2 * z[2] * self.sfc_coeff[6]
            + 2 * z[1] * self.sfc_coeff[8]
            + 2 * self.sfc_coeff[9] * throttle
        )
        dsfc_dthrottle *= self.throttle_coeff
        return dsfc_dthrottle

    def compute_dsfc_dh(self, z, adim_throttle):
        ...
        dsfc_dh = (
            self.sfc_coeff[2]
            + 2 * z[2] * self.sfc_coeff[5]
            + +2 * self.sfc_coeff[7] * z[1]
            + 2 * throttle * self.sfc_coeff[8]
        )
        return dsfc_dh

    def compute_dsfc_dmach(self, z, adim_throttle):
        ...
        dsfc_dmach = (
            self.sfc_coeff[1]
            + 2 * self.sfc_coeff[4] * z[2]
            + 2 * z[1] * self.sfc_coeff[5]
    ...
    def __compute_dadimh_dh(self, z):
        """Compute partial derivative of adim throttle of polynomial function wrt
        altitude.
        ...
        :rtype: numpy array
        """
        return self.base.derive_normalize_s(self.h_initial, z[1])

    def __compute_dadimmach_dmach(self, z):
        ...
        :rtype: numpy array
        """
        return self.base.derive_normalize_s(self.mach_initial, z[2])
    ...
    def __initialize_jacobian(self, true_cstr):
        ...
        jacobian["y_3"]["z"] = zeros((3, 6), dtype=self.dtype)
        ...
        jacobian["g_3"]["z"] = zeros((n_constraints, 6), dtype=self.dtype)
        jacobian["g_3"]["y_23"] = zeros((n_constraints, 1), dtype=self.dtype)

        return jacobian

    def derive_blackbox_propulsion(self, z, y_23, x_3, true_cstr=False):
        ...
        # dSFC_dthrottle
        jacobian["y_3"]["x_3"][0, 0] = self.compute_dsfc_dthrottle(z, x_3[0])
        ...
        # dSFC_dh
        jacobian["y_3"]["z"][0, 1] = self.compute_dsfc_dh(z, x_3[0])
        # dESF_dh= 0.0
        # dengineweight_dh= 0.0

        # dSFC_dM
        jacobian["y_3"]["z"][0, 2] = self.compute_dsfc_dmach(z, x_3[0])
        # dESF_dM= 0.0
        # dengineweight_dM= 0.0

        #        jacobian['y_3']['z'][:, 2:] = 0.0
        ...
        #        dadimtemp_dtemp = self.__compute_dadimtemp_dtemp(z)
        ...
        dg_3_dx_3[2, 0] = self.compute_dthrconst_dthrottle(z)

        # g_3[0, :] = ESF
        dg_3_dxs[0, :] = jacobian["y_3"]["z"][2, :]

        # dtemp_d(t/c)= 0.0
        # d(throttle-throttle_ua)_d(t/c)= 0.0

        # dtemp_dh
        dg_3_dxs[1, 1] = self.__compute_dadimh_dh(z) * (
            ai_coeff[1]
        ...
        dg_3_dxs[2, 1] = self.compute_dthrcons_dh(z, x_3[0])

        # dtemp_dM
        dg_3_dxs[1, 2] = self.__compute_dadimmach_dmach(z) * (
            ai_coeff[0]
        ...
        dg_3_dxs[2, 2] = self.compute_dthrconst_dmach(z, x_3[0])
