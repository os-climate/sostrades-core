...
class ArchiBuilder(SoSDisciplineBuilder):
        ...
        for namespace in self.builder_dict.keys():
            if namespace in activ_builder_dict:
                ...
                for builder in activ_builder_dict[namespace]:
                    ...
                    # We had to comment the lines below because of the side effects generated by
                    # saving archi_builder children in its sos_disciplines attribute
                    # TODO: Try to streamline the code so that each architecteure (archi_builer,
                    # scatter, sos_coupling...) can save its children in the
                    # sos_disciplines attribute so that the code can be more
                    # generic
            ...
            # clean builders not activated anymore
                ...
                # if scatter in self.archi_disciplines[namespace],
                # clean children of scatter
                for disc in self.archi_disciplines[namespace]:
                    if isinstance(disc, SoSDisciplineScatter):
                        disc.clean_scattered_disciplines([])
                self.ee.factory.clean_discipline_list(
                    self.archi_disciplines[namespace])
    ...
    def build_scatter_of_scatter(self, namespace, args, builder_name, builder_on_first_scatter):
        ...
        TO DO  : make this action recursive ! 
        '''
        if args[1][0] == 'scatter':
    ...
    def create_activation_df(self, archi_df):
        ...
        # both in current and parent columns in the archi_df (cf build_architecture method)
        root_node = False
    ...
    # TODO: this method is unused for the moment. It is the cleaning method
    # that comes from sos_coupling. Once streamlined (uniformiser) it can be
    # used in a generic way and move to the sosdisciplinebuilder class

    def remove_discipline_list_backup(self, disc_list):
        ''' remove one discipline from coupling
        '''
        for disc in disc_list:
            # pass
            # self.ee.factory.remove_discipline(disc)
            self.ee.root_process.sos_disciplines.remove(disc)
            disc.clean_dm_from_disc()
            self.ee.ns_manager.remove_dependencies_after_disc_deletion(
                disc, self.disc_id)
