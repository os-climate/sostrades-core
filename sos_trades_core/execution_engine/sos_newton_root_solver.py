...
class NewtonRootSolver(SoSEval):
    ...
    def __init__(self, sos_name, ee, residual_builders, residual_infos, associated_namespaces=None):
    ...
    def update_gems_grammar_with_data_io(self):
        ...
        # delete keys from input grammar if intersection between input and
        # output is not None
        intersection_grammar = list(
            set(list_keys_in_grammar) & set(list_keys_out_grammar))
        if len(intersection_grammar) != 0:
            filtered_inputs = {k: v for k, v in self.sos_disciplines[0].input_grammar._names_to_types.items(
            ) if k not in intersection_grammar}
            # update input grammar with children input grammar not in
            # output_grammar
            self.input_grammar._names_to_types.update(filtered_inputs)
        else:
            self.input_grammar.update_from(
                self.sos_disciplines[0].input_grammar)
