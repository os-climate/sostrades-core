...
class SimpleCustomDriver(SoSWrapp):
    ...
    def run(self):
        input_data = {}
        input_data_names = self.attributes['sub_mdo_discipline'].input_grammar.get_data_names()
        if len(input_data_names) > 0:
            input_data = self.get_sosdisc_inputs(keys=input_data_names, in_dict=True, full_name_keys=True)
        sub_disc_local_data = self.attributes['sub_mdo_discipline'].execute(input_data)
