...
class SoSMDOScenarioAdapter(MDOScenarioAdapter):
    ...
    def _run(self) -> None:
        ...
        if not self.desactivate_optim_out_storage:
            ...
            self.io.data.update({
                next(key for key in self.get_output_data_names() if self.POST_PROC_MDO_DATA in key): post_processing_mdo_data
            })
    ...
    def update_design_space_out(self):
        ...
        for var_name in l_variables:
            ...
            if full_name_var in self.activated_variables:
                value_x_opt = [self.formulation.design_space.get_current_value([full_name_var])]
        ...
        self.local_data.update({
            next(key for key in self.get_output_data_names() if 'design_space_out' in key): design_space
        })
    ...
    def update_post_processing_df(self):
        ...
        constraints_names = [constraint.name for constraint in self.formulation.optimization_problem.constraints]
        ...
        return {
            ...
            "variables": {
                ...
                for var in self.design_space.variable_names
            },
            "constraints": {
