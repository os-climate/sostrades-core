...
class SchemaTypeMismatch(SchemaMismatch):
    def __init__(self, name, desired_type):
        SchemaMismatch.__init__(
            self, '{0} must be {1}'.format(name, desired_type))
    ...
    def make_range_validator(opt):
        ...
        def validate_range(value, name='value'):
            if not check_range(value):
                ...
                raise SchemaRangeMismatch(
                    name + ' must be in range ' + range_str)
...
class Factory(object):
    ...
    def expand_uri(self, type_name):
        ...
        if not m:
            raise ValueError(
                "couldn't understand type name '{0}'".format(type_name))

        prefix, suffix = m.groups()

        if prefix not in self.prefix_registry:
            raise KeyError(
                "unknown prefix '{0}' in type name '{1}'".format(
                    prefix, type_name)
            )
    ...
    def add_prefix(self, name, base):
        if self.prefix_registry.get(name):
            raise SchemaError(
                "the prefix '{0}' is already registered".format(name))
    ...
    def learn_type(self, uri, schema):
        if self.type_registry.get(uri):
            raise SchemaError(
                "tried to learn type for already-registered uri {0}".format(uri))
    ...
    def make_schema(self, schema):
        ...
        if not self.type_registry.get(uri):
            raise SchemaError("unknown type {0}".format(uri))
        ...
        if isinstance(type_class, dict):
            if not {'type'}.issuperset(schema):
                raise SchemaError(
                    'composed type does not take check arguments')
...
class _CoreType(object):
    ...
    def __init__(self, schema, rx):
        ...
        if not {'type'}.issuperset(schema):
            raise SchemaError(
                'unknown parameter for //{0}'.format(self.subname()))
...
class IntType(_CoreType):
    ...
    def __init__(self, schema, rx):
        ...
        self.value = None
        if 'value' in schema:
            if not isinstance(schema['value'],
                              Number) or schema['value'] % 1 != 0:
