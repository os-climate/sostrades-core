...
class SobieskiAerodynamics(object):
    ...
    def __set_coeff_drag_f3(self, z):
        """Setting of flags used for determination of polynomial coefficients.

        :param z: global design variables
        :type z: numpy array
        :returns: s_initial, s_new, flag, bound
        ...
        s_new = array([z[0]], dtype=self.dtype)
        flag = array([1], dtype=self.dtype)
        bound = array([0.25], dtype=self.dtype)
        return s_initial, s_new, flag, bound

    def compute_k_aero(self, z):
        ...
        #        if z[2] >= 1:  # Mach
        #            k = (z[2] ** 2 - 1) * \
        #                self.math.cos(z[4] * DEG_TO_RAD) / \
        #                (4.0 * self.math.sqrt(z[4] ** 2 - 1) - 2)
        #        else:
        #            k = 1. / (self.math.pi * 0.8 * z[3])
        k_aero = (
            (z[2] ** 2 - 1)
            * self.math.cos(z[4] * DEG_TO_RAD)
            / (4.0 * self.math.sqrt(z[4] ** 2 - 1) - 2)
    ...
    #     @staticmethod
    #     def compute_dk_aero_dAR(self, z):
    #         #        if z[2] >= 1:
    #         #            dk_dAR = 0.0
    #         #        else:
    ...
    def compute_dk_aero_dsweep(z):
        ...
        """
        #        if z[2] >= 1:
        #            dk_dsweep = -DEG_TO_RAD * (z[2] ** 2 - 1) * \
        #                self.math.sin(z[4] * DEG_TO_RAD) / \
        #                (4.0 * self.math.sqrt(z[4] ** 2 - 1) - 2)
        #        else:
        #            dk_dsweep = 0.0
        u_velo = (z[2] * z[2] - 1.0) * cos(z[4] * DEG_TO_RAD)
        up_velo = (
            -DEG_TO_RAD
            * (z[2] * z[2] - 1.0)
            * sin(z[4] * DEG_TO_RAD)
        )
        v_velo = 4.0 * sqrt(z[4] * z[4] - 1.0) - 2.0
        vp_velo = 4.0 * z[4] * (z[4] * z[4] - 1.0) ** -0.5
        dk_aero_dsweep = (up_velo * v_velo - u_velo * vp_velo) / v_velo ** 2
        return dk_aero_dsweep

    @staticmethod
    def compute_dk_aero_dmach(z):
        ...
        """
        # if z[2] >= 1:
        #     dk_dM = abs(z[3]) * (2.0 * abs(z[2])) * \
        #             np.cos(z[4] * np.pi / 180.) /
        # (4. * abs(z[3]) * \
        #             np.sqrt(abs(z[4]**2 - 1.) - 2.))
        # else:
        #     dk_dM = 0.0
        dk_aero_dmach = (
            (2.0 * z[2])
            * cos(z[4] * pi / 180.0)
            / (4.0 * sqrt(z[4] ** 2 - 1.0) - 2.0)
        )
    ...
    def compute_dadimtaper_dtaper(self, z):
    ...
    def compute_cd_min(self, z, fo1):
        ...
        return (
            ...
            + 3.05
            * z[0] ** (5.0 / 3.0)
            * (self.math.cos(z[4] * DEG_TO_RAD)) ** 1.5
        )

    @staticmethod
    def compute_dcdmin_dsweep(z):
    ...
    def compute_cd(self, z, lift_coeff, fo1, fo2):
        ...
        :rtype: numpy array
        """
        k_aero = self.compute_k_aero(z)
        cdmin = self.compute_cd_min(z, fo1)
        return fo2 * (cdmin + k_aero * lift_coeff * lift_coeff)

    def compute_drag(self, drag_coeff, z):
        ...
        :rtype: numpy array
        """
        return self.compute_force(drag_coeff, z[5], z[2], z[1])

    def compute_dcd_dsweep(self, z, lift_coeff, fo2):
        ...
        :rtype: numpy array
        """
        dcdmin_dsweep = self.compute_dcdmin_dsweep(z)
        dk_dsweep = self.compute_dk_aero_dsweep(z)
        return fo2 * (dcdmin_dsweep + lift_coeff * lift_coeff * dk_dsweep)

    def compute_dcd_dsref(self, z, y_12, fo2):
        """Computation of derivative of drag coefficient wrt sweep.
        ...
        """
        k_aero = self.compute_k_aero(z)
        lift_coeff = self.compute_cl(z, y_12)
        dcl_dsref = self.compute_dcl_dsref(z, y_12)
        return 2 * k_aero * lift_coeff * dcl_dsref * fo2

    def compute_dcd_dmach(self, z, y_12, fo2):
        ...
        """
        k_aero = self.compute_k_aero(z)
        dk_dmach = self.compute_dk_aero_dmach(z)
        lift_coeff = self.compute_cl(z, y_12)
        dcl_dmach = self.compute_dcl_dmach(z, y_12)
        return (2.0 * k_aero * dcl_dmach + lift_coeff * dk_dmach) * lift_coeff * fo2

    def compute_cl(self, z, y_12):
        ...
        :rtype: numpy array
        """
        return self.compute_adim_coeff(y_12[0], z[5], z[2], z[1])

    def compute_dcl_dh(self, z, y_12):
        ...
        :rtype: numpy array
        """
        rhov2 = self.compute_rhov2(z)
        drhov2_dh = self.compute_drhov2_dh(z)
        return -2 * y_12[0] / z[5] * drhov2_dh / (rhov2 ** 2)

    def compute_dcl_dmach(self, z, y_12):
        ...
        :rtype: numpy array
        """
        rho, velocity = self.compute_rho_v(z[2], z[1])
        dv_dmach = self.compute_dv_dmach(z[1])
        dcl_dmach = (
            -4.0
            * y_12[0]
            * dv_dmach
            / (rho * z[5] * velocity * velocity * velocity)
        )
        return dcl_dmach

    def compute_dcl_dsref(self, z, y_12):
        ...
        :rtype: numpy array
        """
        return -self.compute_cl(z, y_12) / z[5]

    def compute_rhov2(self, z):
    ...
    def compute_drhov2_dh(self, z):
        ...
        """
        rho, velocity = self.compute_rho_v(z[2], z[1])
        drho_dh, dv_dh = self.compute_drho_dh_dv_dh(z[2], z[1])
        return drho_dh * velocity * velocity + 2.0 * rho * dv_dh * velocity

    def compute_drhov2_dmach(self, z):
    ...
    def blackbox_aerodynamics(self, z, y_12, y_32, x_2, true_cstr=False):
        ...
        drag_coeff = self.compute_cd(z, lift_coeff, fo1, fo2)
        drag = self.compute_drag(drag_coeff, z)
        ...
        s_initial3, s_new3, flag3, bound3 = self.__set_coeff_drag_f3(z)
    ...
    def __set_coupling_jacobian(jacobian):
        ...
        jacobian["y_21"]["z"] = atleast_2d(jacobian["y_2"]["z"][0, :])
        ...
        jacobian["y_23"]["z"] = atleast_2d(jacobian["y_2"]["z"][1, :])
        ...
        jacobian["y_24"]["z"] = atleast_2d(jacobian["y_2"]["z"][2, :])
    ...
    def __initialize_jacobian(self):
        ...
        jacobian["y_2"]["z"] = zeros((3, 6), dtype=self.dtype)
        ...
        jacobian["g_2"]["z"] = zeros((1, 6), dtype=self.dtype)
        jacobian["g_2"]["y_12"] = zeros((1, 2), dtype=self.dtype)
        jacobian["g_2"]["y_32"] = zeros((1, 1), dtype=self.dtype)
        return jacobian

    def derive_blackbox_aerodynamics(self, z, y_12, y_32, x_2):
        """This function calculates drag and lift to drag ratio of A/C.
        ...
        drag_coeff_min = self.compute_cd_min(z, fo1)
        ...
        # dLift/dZ= 0.0
        # dDrag/d(t/c)
        dy2dxs = dyn_force * fo2 * 3.05 * 5.0 / 3.0 * z[0] ** (2.0 / 3.0)
        dy2dxs *= (self.math.cos(z[4] * DEG_TO_RAD)) ** 1.5
        jacobian["y_2"]["z"][1, 0] = dy2dxs

        # d(Lift/Drag)/d(t/c)

        # d(Drag)/dh
        drhov2_dh = self.compute_drhov2_dh(z)
        dcl_dh = self.compute_dcl_dh(z, y_12)
        dy2dxs = 0.5 * z[5] * fo2 * drag_coeff_min * drhov2_dh
        add_der = cl2 * drhov2_dh + rhov2 * 2.0 * lift_coeff * dcl_dh
        dy2dxs += 0.5 * z[5] * k_aero * fo2 * add_der
        jacobian["y_2"]["z"][1, 1] = dy2dxs
        # d(Lift/Drag)/dh

        # d(Drag)/dM
        dcd_dmach = self.compute_dcd_dmach(z, y_12, fo2)
        drhov2_dmach = self.compute_drhov2_dmach(z)
        jacobian["y_2"]["z"][1, 2] = (
            0.5 * z[5] * (drag_coeff * drhov2_dmach + rhov2 * dcd_dmach)
        )
        # d(Lift/Drag)/dM

        # d(Drag)/dAR
        #         dk_dAR = self.compute_dk_aero_dAR(z) # = 0.0
        #         jacobian['y_2']['z'][1, 3] = dyn_force * dk_dAR * fo1 * cl2
        # d(Lift/Drag)/dAR

        # d(Drag)/dsweep
        dcd_dsweep = self.compute_dcd_dsweep(z, lift_coeff, fo2)
        jacobian["y_2"]["z"][1, 4] = dyn_force * dcd_dsweep

        # d(Drag)/dsref
        dcd_dsef = self.compute_dcd_dsref(z, y_12, fo2)
        dy2dxs = drag / z[5] + dyn_force * dcd_dsef
        jacobian["y_2"]["z"][1, 5] = dy2dxs

        for i in range(6):
            dy2dxs0i = jacobian["y_2"]["z"][0, i]
            dy2dxs1i = jacobian["y_2"]["z"][1, i]
            dlod = self.__derive_liftoverdrag(cl_cd, dy2dxs0i, dy2dxs1i, inv_drag)
            jacobian["y_2"]["z"][2, i] = dlod
        ...
        dadimtaper_dtaper = self.compute_dadimtaper_dtaper(z)
        jacobian["g_2"]["z"][0, 0] = dadimtaper_dtaper * (
            ai_coeff3[0] + aij_coeff3[0, 0] * s_shifted3[0]
        )
        #        jacobian['g_2']['z'][0, 1:] = 0.0
