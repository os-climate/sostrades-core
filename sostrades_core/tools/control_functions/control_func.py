...
def lagrangian_objective_func(self):
    ns_objective = self.disciplines[0].dm.get_all_namespaces_from_var_name(
        'objective_lagrangian')[0]

    condition = self.disciplines[0].dm.get_value(ns_objective)[0] > 1.0e10
...
def max_ite_func(self):
    try:
        ns_objective_list = self.disciplines[0].dm.get_all_namespaces_from_var_name(
            'optim_output_df')
        scenario_name = self.disciplines[0].get_disc_full_name(
        ).replace('.' + self.disciplines[0].sos_name, '')

        ns_objective = [
            ns_obj for ns_obj in ns_objective_list if ns_obj.startswith(scenario_name)][0]
        condition = self.disciplines[0].dm.get_value(
            ns_objective)['iteration'].values.max() < 10

        print('condition on iterations :', condition, 'objective <10  :',
              self.disciplines[0].dm.get_value(
