...
class DoeEval(SoSWrapp):
    ...
    #     self.attributes['selected_outputs'] = []
    #     self.attributes['selected_inputs'] = []
    ...
    #                                       == True]['full_name']
    #         if set(selected_inputs.tolist()) != set(self.attributes['selected_inputs']):
    #             selected_inputs_has_changed = True
    #             self.attributes['selected_inputs'] = selected_inputs.tolist()
    #         self.attributes['selected_outputs'] = selected_outputs.tolist()
    #
    ...
    #             # output
    #
    #             for out_var in self.attributes['eval_out_list']:
    #                 dynamic_outputs.update(
    #                     {f'{out_var.split(self.attributes["study_name"] + ".")[1]}_dict': {'type': 'dict', 'visibility': 'Shared',
    #                                                                             'namespace': 'ns_doe'}})
    #
    #             if algo_name == "CustomDOE":
    #                 default_custom_dataframe = pd.DataFrame(
    #                     [[NaN for input in range(len(self.attributes['selected_inputs']))]], columns=self.attributes['selected_inputs'])
    #                 dataframe_descriptor = {}
    #                 for i, key in enumerate(self.attributes['selected_inputs']):
    #                     cle = key
    #                     var = tuple([self.attributes['dm'].get_data(
    #                         self.attributes['eval_in_list'][i], 'type'), None, True])
    ...
    #
    #                                                      'lower_bnd': [[0.0, 0.0] if self.attributes['dm'].get_data(var,
    #                                                                                                      'type') == 'array' else 0.0
    #                                                                    for var in self.attributes['eval_in_list']],
    #                                                      'upper_bnd': [[10.0, 10.0] if self.attributes['dm'].get_data(var,
    ...
    #             if (len(self.attributes['selected_inputs']) > 0) and (any([self.MULTIPLIER_PARTICULE in val for val in self.attributes['selected_inputs']])):
    ...
    def set_eval_in_out_lists(self, in_list, out_list):
        ...
        self.attributes['eval_in_list'] = [
            f'{self.attributes["study_name"]}.{element}' for element in in_list]
        self.attributes['eval_out_list'] = [
            f'{self.attributes["study_name"]}.{element}' for element in out_list]
    ...
    def create_generic_multipliers_dynamic_input(self):
        dynamic_inputs_list = []
        for selected_in in self.attributes['selected_inputs']:
            if self.MULTIPLIER_PARTICULE in selected_in:
                ...
                # if
                if len(self.attributes['dm'].get_all_namespaces_from_var_name(origin_var_name)) > 1:
                    LOGGER.exception(
                        'Multiplier name selected already exists!')
                origin_var_fullname = self.attributes['dm'].get_all_namespaces_from_var_name(origin_var_name)[
                    0]
                origin_var_ns = self.attributes['dm'].get_data(
                ...
                dynamic_inputs_list.append(
                    {
                        f'{multiplier_name}': {
                            ...
                            'unit': self.attributes['dm'].get_data(origin_var_fullname).get('unit', '-'),
                            'default': 100
                        }
                    }
                )
    ...
    #                     full_id.split(self.attributes["study_name"] + ".")[1])
    #
    ...
    #                      self.POSSIBLE_VALUES, self.custom_order_possible_algorithms(self.attributes['doe_factory'].algorithms))
