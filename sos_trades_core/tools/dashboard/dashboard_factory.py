            ...
            for row in rows:
                for block in row:
                    if block.get("content_type") == "POST_PROCESSING":
                        try:
                            discipline = block.get('content_namespace').format(study_name=execution_engine.study_name)

                            post_processing_bundle = all_post_processings.get(discipline)
                            post_processings = post_processing_bundle[0].post_processings
                            content = post_processings[block.get("graph_index", 0)]
