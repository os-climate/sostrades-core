...
class TornadoChartAnalysis(SoSWrapp):
    ...
    @staticmethod
    def __make_tornado_chart(variation_df:pd.DataFrame, selected_inputs:list[str], output_variable_name:str) -> TwoAxesInstanciatedChart:
        """
        Make a tornado chart
        :params: variation_df, the output variation df
        :params: selected_inputs, the selected inputs
        :params: output_variable_name, the name of the output variable
        """
        ...
        def get_output_variation_value(input_name:str, variation_value:float) -> float:
            ...
            """
            result = variation_df[(variation_df[TornadoChartAnalysis.INPUT_COL] == input_name) & (variation_df[TornadoChartAnalysis.VARIATION_INPUT_COL] == variation_value)][TornadoChartAnalysis.VARIATION_OUTPUT_COL]
