...
This input sample is either generated based on a selected algorithm of DoE (lhs, full factorial, ...), or provided by the user (Custom DoE).

The user selects the input variables that he wants to vary for the DoE by using the "Evaluated Inputs selector". 
Those variables are then the columns of the input sample:

- In case of a sample generated by an algorithm (lhs, full factorial, ...), the variables should be in the following numerical types : ['float', 'array', 'int']. 
...
- In case of a Custom DoE, the sample may have more variables (for its columns) than what is selected in the "Evaluated Inputs" selector. 
  However, it should have ALL the selected variables by the "Evaluated Inputs" selector. 
...
- An "Input Samples Data" table of data is provided in the outputs of this driver (it corresponds to the outcome of the already mentioned "Step 1"). 
  The variables in header of the columns are provided in alphabetical order and are only the used variables for the DoE. 
  In case of a Custom DoE with more columns than those selected in the "Evaluated Inputs" selector, only selected Eval input variables are kept in this dataset table.
...
Those reference values will be then used for each variable that is not a selected variable to vary in the DoE.
...
The main parameters associated with the DoE driver are: Algo of sampling, Evaluated Inputs, and Evaluated Outputs.
![Main inputs](./doe_eval_png/DoE_Eval_04.PNG)

## GUI inputs: Inputs/Outputs selection

The user needs to select the variables he wants to be varied for his DoE by using the "Evaluated Inputs" selector.
...
Remark: 
- In the GUI: the variables are sorted in alphabetical order. 
- In python script: 
  - in case of a Custom DoE, the user can input a "Evaluated Inputs" dataframe with non sorted variables: however the tool will sort them alphabetically.
  - if it is not a Custom DoE, the user needs to input a "Evaluated Inputs" dataframe with sorted variables
...
- He also will see several "dict of output variables" (empty and read only here) for each selected output variable: here "obj_dict" as we have selected the obj variable.
After a run, each dict of output variable will have the same size as the "Input Samples Data".
...
Warning: 
- The "Custom sample dataframe" CAN have more column variables as the selected variables in the "Evaluated Inputs" selector: the extra variables will not be taken into account. 
...
- The "Custom sample dataframe" MUST HAVE at least all the selected variables in the "Evaluated Inputs" selector: if not then it will rise an exception error with a message telling you what are the missing variables
...
Warning: 
- The "Design Space" must have the same variables as the selected variables in "Evaluated Inputs" selector and they have to be in alphabetic order.
...
The user NEEDs also to provide a reference value for each input variable of the nested subprocess (except for variables already provided by a default value).
...
## GUI inputs: run
When all inputs data have been provided, and saved, the user can run the DoE, that goes from the configuration state, to Pending and Done (or Failure with error message if something goes wrong). 
...
The "Input Samples Data" is the table of data on wich there will be a loop on each rows : 
the columns of this table are sorted alphabetically and are the same as the list of selected "Evaluated Inputs" (sorted in alphabetical order).

For this sellar example, the "Input Samples Data" is the following:
...
- W. Tinsson. Cours 1 et 2 Plans d'exp\`erience 
        ([link1](https://reseau-mexico.fr/sites/reseau-mexico.fr/files/PECNUM_cours1_tinsson.pdf), [link2](https://reseau-mexico.fr/sites/reseau-mexico.fr/files/PECNUM_cours2_tinsson.pdf))