...
class ArchiBuilder(SoSDisciplineBuilder):
    ...
    def __init__(self, sos_name, ee, architecture_df, custom_vb_folder_list=None, associated_namespaces=None):
        """
        Constructor
        """
    ...
    def is_builder_activated(self, namespace, builder_name):
        ...
        """
        if (
            self.ACTIVATION_DF in self._data_in
    ...
    def set_input_name_value(self, scatter_map, input_name):
        ...
        if input_name not in self._data_in:
            ...
            self.ee.dm.set_data(
                self.get_var_full_name(input_name, self._data_in),
                self.VALUE,
                input_value,
            )
            self.ee.dm.set_data(
    ...
    def create_activation_df(self, archi_df):
        ...
        if self.ACTIVATION_DF not in self._data_in:
            ...
            if not self.default_activation_df.equals(
                self.dm.get_data(
                    self.get_var_full_name(self.ACTIVATION_DF, self._data_in),
                    SoSDisciplineBuilder.DEFAULT,
                )
            ):
                self._data_in[self.ACTIVATION_DF][
                    self.VALUE
