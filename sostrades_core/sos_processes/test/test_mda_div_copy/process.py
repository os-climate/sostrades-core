...
class ProcessBuilder(BaseProcessBuilder):
    ...
    _ontology_data = {
        'label': 'Core Test MDA Div Copy Process',
        'description': '',
        'category': '',
        'version': '',
    }
    def get_builders(self):
        ...
        disc_dir = 'sostrades_core.sos_wrapping.test_discs.mda_div_idf.'
        mods_dict = {
            'SellarCopy': disc_dir + 'SellarCopy',
            'Sellar_Problem': disc_dir + 'SellarProblem',
        }
        list_builders = self.create_builder_list(
        ...
        builder_sellar = self.ee.factory.get_builder_from_process(
            'sostrades_core.sos_processes.test', 'test_mda_div')
        m = [list_builders[0]]
