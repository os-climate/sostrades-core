    ...
    def setup_dynamic_inputs_for_sensitivity_analysis_method(self, dynamic_inputs, proxy):
        ...
        if proxy.EVAL_INPUTS in disc_in:
            ...
            if eval_inputs is not None:
                ...
                design_scenario_mapping_descriptor.update({name: ("float", None, False) for name in selected_inputs})
                dynamic_inputs.update(
                    {
                        self.SCENARIO_VARIABLE_VARIATIONS: {
    ...
    def _generate_samples(self, variation_list, dict_of_value, proxy):
        ...
        for name, value in dict_of_value.items():
            ...
            if (
                ...
                and bool([isinstance(percent, float) for percent in variation_list])
            ):
                ...
                samples_dict[scenario_names_str].extend(
                    [f"scenario_{i}" for i in range(current_sc, nb_scenario + current_sc)]
                )
                ...
                scenario_variations_dict[scenario_names_str].extend(
                    [f"scenario_{i}" for i in range(current_sc, nb_scenario + current_sc)]
                )
        ...
        disc_in = proxy.get_data_in()
        proxy.dm.set_data(
            proxy.get_var_full_name(self.SCENARIO_VARIABLE_VARIATIONS, disc_in),
            proxy.VALUE,
