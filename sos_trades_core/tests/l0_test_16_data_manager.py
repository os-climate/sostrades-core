...
from sos_trades_core.tests.l0_test_06_dict_pickle_import_export_dm import init_dict
from sos_trades_core.tools.rw.load_dump_dm_data import CryptedLoadDump, DirectLoadDump
...
class TestDataManagerGenerator(unittest.TestCase):
    ...
    def test_01_load_DM(self):
        ...
        ref_dict = {ns_2 + '.x': init_dict('float'),
                    ...
                    ns_2 + '.reset_history_each_run': init_dict('string'),
                    ...
                    ns_2 + '.condition_func': init_dict('string'),
        ...
        for key in ref_dict:
            if ref_dict[key][SoSDiscipline.TYPE] != "dataframe":
    ...
    def test_02_DM_with_soscoupling(self):
        ...
        for disc in status_dict_from_dm.values():
            self.assertEqual(list(disc.values())[0], SoSDiscipline.STATUS_DONE)
...
go to ref dir (sos_trades_core\tests\data\ref_output\<STUDY_DIR>)
import pickle
