...
class ProcessBuilder(BaseProcessBuilder):
    ...
    def get_builders(self):
        ...
        mda_builder = self.ee.factory.get_builder_from_process(
            'sostrades_core.sos_processes.test', 'test_mda_div_copy')
        coupling_builder = self.ee.factory.create_builder_coupling('SellarEval')
        coupling_builder.set_builder_info('cls_builder', mda_builder)
    
        ns_dict = {'ns_OptimSellar': self.ee.study_name + 
                   '.SellarOptimScenario.SellarEval.SellarCoupling'}
