...
class TestSimpleMultiScenario(unittest.TestCase):
    ...
    def _test_01_multi_instance_configuration_from_df_with_eval_outputs_as_hard_input(self):
        ...
        self.assertIn('Gather.Disc1.indicator_gather', ms_disc_out)
        self.assertIn('Gather.y_gather', ms_disc_out)
        self.assertNotIn('Gather.o_gather', ms_disc_out)

        self.exec_eng.execute()

        # check output correctness
        y_gather = self.exec_eng.dm.get_value(
            'MyCase.multi_scenarios.Gather.y_gather')
        indicator_gather = self.exec_eng.dm.get_value(
            'MyCase.multi_scenarios.Gather.Disc1.indicator_gather')
