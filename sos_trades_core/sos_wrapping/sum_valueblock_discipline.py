...
class SumValueBlockDiscipline(ValueBlockDiscipline):
    ...
    def build_dynamic_io(self):
        ...
        """
        dynamic_inputs, dynamic_outputs = ValueBlockDiscipline.build_dynamic_io(self)
        self.input_to_sum = {}
            ...
            if not input_to_sum.endswith('_gather'):
                ...
                if len(list(sub_input_dict.keys())) >= 2:
                    ...
                    if type_input in ['float', 'int', 'array']:
                        output_dict[input_to_sum] = sum(sub_input_dict.values())
                        ...
                        for ref_key, ref_value in ref_dict.items():
                            ...
                            if (
                                isinstance(ref_value, tuple(self.STANDARD_TYPES))
                                and check_key == []
                            ):
                                values_to_sum = [
                    ...
                    output_dict[input_to_sum] = list(sub_input_dict.values())[0]
                else:
                    raise Exception(
