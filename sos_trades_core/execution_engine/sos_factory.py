...
class SosFactory:
    ...
    GENERIC_MODS_PATH = 'sos_trades_core.sos_wrapping.analysis_discs'
    ...
    def create_multi_scatter_builder_from_list(self, map_name, builder_list, autogather=False, path_sum=None):
        '''
...
# right place : {self.__current_discipline.get_disc_full_name()}')
    ...
    def add_discipline_list(self, disciplines):
        ...
        self.__sos_disciplines.extend(disciplines)
    ...
    def get_builder_from_process(self, repo, mod_id, **args):
        ...
        pb_cls = getattr(import_module(SosFactory.build_module_name(repo, mod_id)),
    ...
    def create_evaluator_builder(self, sos_name, eval_type, cls_builder):
        ...
        if eval_type == 'sensitivity':
            module_struct_list = f'{self.GENERIC_MODS_PATH}.sensitivity_analysis.SensitivityAnalysis'
        elif eval_type == 'gradient':
            module_struct_list = f'{self.GENERIC_MODS_PATH}.gradient_analysis.GradientAnalysis'
        elif eval_type == 'FORM':
    ...
    def create_value_block_builder(self, builder_name, own_map_name, connected_map_name, associated_builder_list, autogather=False, builder_child_path=None):
        ...
        builder.set_builder_info(
            'own_map_name', own_map_name)
        builder.set_builder_info(
            'child_map_name', connected_map_name)
        builder.set_builder_info(
    ...
    def create_architecture_builder(self, builder_name, architecture_df):
        '''
    ...
    def create_multi_scenario_builder(self, sos_name, map_name, cls_builder, autogather=False, gather_node=None, business_post_proc=False):
        ...
        if autogather:
            ...
            mod_path_multi_scatter = f'{self.EE_PATH}.sos_multi_scatter_builder.SoSMultiScatterBuilder'
    ...
    def create_very_simple_multi_scenario_builder(self, sos_name, map_name, cls_builder, autogather=False, gather_node=None, business_post_proc=False):
        ...
        '''
        builder_list = self.convert_builder_to_list(cls_builder)
        mod_path = f'{self.EE_PATH}.sos_very_simple_multi_scenario.SoSVerySimpleMultiScenario'
        cls = self.get_disc_class_from_module(mod_path)
        builder = SoSBuilder(sos_name, self.__execution_engine, cls)
        builder.set_builder_info(
            'map_name', map_name)
    ...
    def create_optim_builder(self, sos_name, cls_builder):
        ...
        '''
        module_struct_list = 'sos_trades_core.execution_engine.sos_optim_scenario.SoSOptimScenario'
        cls = self.get_disc_class_from_module(module_struct_list)
        builder = SoSBuilder(sos_name, self.__execution_engine, cls)
    ...
    def create_doe_builder(self, sos_name, cls_builder):
        ''' creates the builder of the doe scenario
           ...
           @author: MARGERIT_D
        '''
        module_struct_list = 'sos_trades_core.execution_engine.sos_doe_scenario.SoSDOEScenario'
        cls = self.get_disc_class_from_module(module_struct_list)
        builder = SoSBuilder(sos_name, self.__execution_engine, cls)
        builder.set_builder_info('cls_builder', cls_builder)
        return builder
    ...
    def get_module_class_path(self, class_name, folder_list):
        ...
        for folder in folder_list:
            ...
            sub_module_list = [import_module(
                '.'.join([folder, file.split('.')[0]])) for file in file_list]

            for sub_module in sub_module_list:
                # Find all members of each submodule which are classes
                # belonging to the sub_module
                class_list = [value for value, cls in inspect.getmembers(sub_module) if inspect.isclass(
                    getattr(sub_module, value)) and cls.__module__ == sub_module.__name__]
    ...
    def clean_discipline_list(self, disciplines, current_discipline=None):
        '''
        Clean all disciplines in the sos_disciplines list of the factory and of the current_discipline
        '''
        ...
        for disc in disciplines:
            ...
            if isinstance(disc, SoSDisciplineBuilder):
    ...
    def update_builder_list_with_extra_name(self, extra_name, builder_list=None):
        ...
        for builder in builder_list:
            self.update_builder_with_extra_name(
                builder, extra_name)
